...
use webman_main;
use webman_blob_content_separator;
...
if ($blob_id ne "") {
    ...
    if ($found) {                             
        ...
        if ($file_name =~ /.html/i || $file_name =~ /.htm/i) {
            $web_man_bcs = new webman_blob_content_separator;
            
            $web_man_bcs->set_HTML_BLOB_Content($content);
            
            $content = $web_man_bcs->get_HTML_Code;
        }
