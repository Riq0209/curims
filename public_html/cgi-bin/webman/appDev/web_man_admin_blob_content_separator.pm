package web_man_admin_blob_content_separator;
    ...
    if ($this->{html_blob_content} ne undef) {
        ...
        if ($html_objects[0] ne undef) { ### always return 1 item even there is no html object exist ???
            ...
            for ($i = 0; $i < @html_objects; $i++) {
                ...
                if ($obj_ref_name_old =~ /web_man_blob_content_printer.cgi/ || 
                    $obj_ref_name_old =~ /web_man_blob_content_printer_v2.cgi/) {
                    ...
                    #$this->{html_content} .= "<pre>-" . $obj_ref_name_old . "-</pre>";
                }
                
                if ($this->is_Embedded_BLOB($obj_ref_name_old)) {
                    #print "Found Embedded BLOB:" .  $obj_ref_name_old . "<br>";
                    ...
                    $obj_ref_name_old =~ s/\?/\\\?/;
                    $obj_ref_name_old =~ s/\+/\\\+/g;
                    
                    $temp_content =~ s/$obj_ref_name_old/$obj_ref_name/;
                }
                
                #print "\$obj_ref_name_old = $obj_ref_name_old <br>";
            } 
        }
        ...
        #print $temp_content;
        #print "\$blob_obj_count = $blob_obj_count <br>";
    }
