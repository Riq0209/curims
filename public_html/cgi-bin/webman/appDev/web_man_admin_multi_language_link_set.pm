package web_man_admin_multi_language_link_set;
...
sub process_Content {
    ...
    $this->set_Template_File("./template_admin_multi_language_link_set.html");
...
sub process_DYNAMIC { ### so_type_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    my $hpd = $this->generate_Hidden_POST_Data("link_name_selected_lang dmisn_selected_lang lang_id link_name_multi_lang dmisn_multi_lang link_name dmisn app_name link_id");
...
sub process_LIST { ### so_type_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    $dbihtml->set_SQL("select * from $pre_table_name" . "link_structure where link_id='$link_id'");
    ...
    if ($db_items_num > 0) {
        ...
        my $link_name_translate = $dbu->get_Item("link_name_translate", "link_id lang_id", "$link_id $lang_id");
