package wmcudas_group_multirows_insert;
...
sub run_Task {
    ...
    if ($cgi->param("irn_wmcudas_group") > 0) {
        $this->{row_num} = $cgi->param("irn_wmcudas_group");
        
    } else {
        $cgi->push_Param("irn_wmcudas_group", $this->{row_num})
    }    
    ...
    ### Manually generate the "id_group_36base" unique values
    for (my $idx = 0; $idx < $this->{row_num}; $idx++) {
        if (!$cgi->param_Exist("\$db_id_group_36base_$idx")) {
            ### All group ID have to mainly refered from framework's default group table.
            $dbu->set_Table("webman_wmcudas_group");
            $cgi->push_Param("\$db_id_group_36base_$idx", $dbu->get_Unique_Random_36Base("id_group"));
        }
    }
    ...
    if ($status) {
        ...
        for (my $idx =0; $idx < $this->{row_num}; $idx++) {
            $id_group_in .= "'" . $cgi->param("\$db_id_group_36base_$idx") . "',";
        }
        
        $id_group_in =~ s/,$//;
        
        #$cgi->add_Debug_Text("\$id_group_in = $id_group_in ", __FILE__, __LINE__);
        
        my $sth = $db_conn->prepare("insert into webman_wmcudas_group (id_group, group_name, description) 
                                     select id_group_36base, group_name, description 
                                     from wmcudas_group where id_group_36base in ($id_group_in)");
