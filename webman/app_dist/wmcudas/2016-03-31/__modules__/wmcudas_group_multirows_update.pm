package wmcudas_group_multirows_update;
...
sub run_Task {
    ...
    if ($status) {
        ### Also update groups inside framework's default group table.
        
        my $idx = 0;
        
        while ($cgi->param_Exist("\$db_id_group_36base_$idx")) {
            my $id_group = $cgi->param("\$db_id_group_36base_$idx");
            my $group_name = $cgi->param("\$db_group_name_$idx");
            my $description = $cgi->param("\$db_description_$idx");
            
            ### Update 'group_name' inside framework's default user-group table.
            $dbu->set_Table("webman_wmcudas_group");
            my $group_name_old = $dbu->get_Item("group_name", "id_group", "$id_group");
            
            $dbu->set_Table("webman_wmcudas_user_group");
            $dbu->update_Item("group_name", "$group_name", "group_name", "$group_name_old");
            
            $cgi->add_Debug_Text($dbu->get_SQL, __FILE__, __LINE__, "DATABASE");
            
            my $sql = "update webman_wmcudas_group " .
