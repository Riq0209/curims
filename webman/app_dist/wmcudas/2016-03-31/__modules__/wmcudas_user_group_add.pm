package wmcudas_user_group_add;
...
sub run_Task {
    ...
    my $login_name = $dbu->get_Item("login_name", "id_user_36base", $cgi->param("id_user_36base"));
    $cgi->push_Param("login_name", "$login_name");
    ...
    if ($submission_type eq "parent_list") {
        $cgi->push_Param("task", "wmcudas_user_group_list");        
        $cgi->redirect_Page("index.cgi?$get_data");
        
    } elsif ($submission_type eq "add_selected" && grep(/^id_group_36base_/, @cgi_var_list)) {  
        my $id_item_parent = $cgi->param("id_user_36base");
        
        $cgi->push_Param("\$db_id_user_36base", $id_item_parent);
        ...
        $htmldb->set_Table("wmcudas_usergroup");        

        foreach my $var (@cgi_var_list) {                
            if ($var =~ /^id_group_36base_/) {
                my $id_item_added = $cgi->param_Shift($var);
                
                $cgi->push_Param("\$db_id_group_36base", $id_item_added);

                ### get the current number of selected item to be added
                my $num = $var;
                   $num =~ s/^id_group_36base_//;
                ...
                ### Copy user-group assignment into framework's default user-group table.
                $dbu->set_Table("wmcudas_user");
                my $login_name = $dbu->get_Item("login_name", "id_user_36base", $cgi->param("\$db_id_user_36base"));
                
                $dbu->set_Table("wmcudas_group");
                my $group_name = $dbu->get_Item("group_name", "id_group_36base", $cgi->param("\$db_id_group_36base"));
                ...
                $cgi->param_Shift("\$db_id_group_36base");
            }                
        }
...
sub process_DYNAMIC { ### TE_TYPE_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    if ($te_type_name eq "parent_item_info") {
        ...
        $component->set_SQL("select * from __parent_table_name__ where id_user_36base='$id_item_parent'");
