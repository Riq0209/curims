package wmcudas_group_user_add;
...
sub run_Task {
    ...
    if ($submission_type eq "parent_list") {
        $cgi->push_Param("task", "wmcudas_group_user_list");        
        $cgi->redirect_Page("index.cgi?$get_data");
        
    } elsif ($submission_type eq "add_selected" && grep(/^id_user_36base_/, @cgi_var_list)) {  
        my $id_item_parent = $cgi->param("id_group_36base");
        
        $cgi->push_Param("\$db_id_group_36base", $id_item_parent);
...
sub process_DYNAMIC { ### TE_TYPE_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    if ($te_type_name eq "parent_item_info") {
        ...
        $component->set_SQL("select * from __parent_table_name__ where id_group_36base='$id_item_parent'");
