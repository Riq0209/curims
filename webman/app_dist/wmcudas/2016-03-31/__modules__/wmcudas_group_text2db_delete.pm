package wmcudas_group_text2db_delete;
...
sub run_Task {
    ...
    if ($status) {
        ...
        foreach my $sqlcmd (@sqlcmds) {
            ...
            my $group_name = $parts[1];
               
            $dbu->set_Table("webman_wmcudas_group");
            my $id_group = $dbu->get_Item("id_group", "group_name", "$group_name");
            
            #$cgi->add_Debug_Text("$group_name : $id_group", __FILE__, __LINE__, "TRACING");
            ...
            $sqlcmd =~ s/delete from wmcudas_group /delete from webman_wmcudas_group /;
