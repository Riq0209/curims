package wmcudas_apps_group_user_link;
...
sub customize_TLD {
    ...
    $tld->add_Column("wmcudas_user");
    $tld->add_Column("app_link");
    ...
    for ($i = 0; $i < $tld->get_Row_Num; $i++) { 
        ...
        my $count_item = $dbu->count_Item("id_apps_36base", $tld->get_Data($i, "id_apps_36base"));
        $tld->set_Data($i, "wmcudas_group", $count_item);

        $dbu->set_Table("wmcudas_userapps");
        my $count_item = $dbu->count_Item("id_apps_36base", $tld->get_Data($i, "id_apps_36base"));
        $tld->set_Data($i, "wmcudas_user", $count_item);
        
        my $app_link = "http://" . $tld->get_Data($i, "dns") . $tld->get_Data($i, "path") . 
                       $tld->get_Data($i, "app_name") . "/index.cgi";
        $tld->set_Data($i, "app_link", $app_link);        
