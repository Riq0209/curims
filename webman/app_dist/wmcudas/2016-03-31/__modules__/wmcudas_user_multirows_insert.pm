...
sub run_Task {
    ...
    ### Manually generate the "id_user_36base" unique values
    for (my $idx = 0; $idx < $this->{row_num}; $idx++) {
        if (!$cgi->param_Exist("\$db_id_user_36base_$idx")) {
            ### All user ID have to mainly refered from framework default user table.
            $dbu->set_Table("webman_wmcudas_user");
            $cgi->push_Param("\$db_id_user_36base_$idx", $dbu->get_Unique_Random_36Base("id_user"));
        }
    }    
    ...
    if ($status) {
        ...
        for (my $idx =0; $idx < $this->{row_num}; $idx++) {
            $id_user_in .= "'" . $cgi->param("\$db_id_user_36base_$idx") . "',";
        }
        
        $id_user_in =~ s/,$//;
        
        #$cgi->add_Debug_Text("\$id_user_in = $id_user_in ", __FILE__, __LINE__);
