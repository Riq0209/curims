package wmcudas_apps_multirows_insert;
...
sub run_Task {
    ...
    if ($cgi->param("irn_wmcudas_apps") > 0) {
        $this->{row_num} = $cgi->param("irn_wmcudas_apps");
        
    } else {
        $cgi->push_Param("irn_wmcudas_apps", $this->{row_num})
    }    
