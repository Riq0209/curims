package wmcudas_group_user_list;
...
sub run_Task {
    ...
    my $group_name = $dbu->get_Item("group_name", "id_group_36base", $cgi->param("id_group_36base"));
    $cgi->push_Param("group_name", "$group_name");
...
sub customize_SQL {
    ...
    if (!$cgi->param_Exist("filter_login_name") || $cgi->param("filter_login_name") eq "") {
        $cgi->push_Param("filter_login_name", "\%");
    }
    my $filter_login_name = $cgi->param("filter_login_name");
    $sql_filter .= "login_name like '$filter_login_name' and ";
    
    
    if (!$cgi->param_Exist("filter_full_name") || $cgi->param("filter_full_name") eq "") {
        $cgi->push_Param("filter_full_name", "\%");
    }
    my $filter_full_name = $cgi->param("filter_full_name");
    $sql_filter .= "full_name like '$filter_full_name' and ";
    
    
    if (!$cgi->param_Exist("filter_description") || $cgi->param("filter_description") eq "") {
        $cgi->push_Param("filter_description", "\%");
    }
    my $filter_description = $cgi->param("filter_description");
    $sql_filter .= "description like '$filter_description' and ";
