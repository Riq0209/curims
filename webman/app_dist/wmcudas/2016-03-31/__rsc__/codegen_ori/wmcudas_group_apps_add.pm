package wmcudas_group_apps_add;
...
sub run_Task {
    ...
    if ($submission_type eq "parent_list") {
        $cgi->push_Param("task", "wmcudas_group_apps_list");        
        $cgi->redirect_Page("index.cgi?$get_data");
        
    } elsif ($submission_type eq "add_selected" && grep(/^id_apps_36base_/, @cgi_var_list)) {  
        ...
        $htmldb->set_Table("wmcudas_groupapps");        

        foreach my $var (@cgi_var_list) {                
            if ($var =~ /^id_apps_36base_/) {
                my $id_item_added = $cgi->param_Shift($var);
                
                $cgi->push_Param("\$db_id_apps_36base", $id_item_added);

                ### get the current number of selected item to be added
                my $num = $var;
                   $num =~ s/^id_apps_36base_//;
                ...
                $cgi->param_Shift("\$db_id_apps_36base");
            }                
        }
