package app_admin_user_assign_groups;
...
sub run_Task {
    ...
    if ($task eq "user_add_group") {
        ...
        foreach my $group_name (@group_list) {       
            ...
            my $found_user_group = $dbu->find_Item("login_name group_name", "$login_name_selected $group_name");

            if ($found_group && !$found_user_group) { ### this is not required if $submit eq "Add Selected" but 
        ...
        $cgi->set_Param_Val("task", "user_assign_group"); ### reset task CGI variable
    }
    
    if ($task eq "user_remove_group") {
        my $group_name = $cgi->param_Shift("group_name");
        
        $dbu->set_Table("webman_" . $cgi->param("app_name_in_control") . "_user_group");
        $dbu->delete_Item("login_name group_name", "$login_name_selected $group_name");
...
sub process_DYNAMIC { ### so_type_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    if ($te_type_name eq "form_hidden_field") {
    ...
    } elsif ($te_type_name eq "group_multi_selection" && $cgi->param("open_list")) {
        my $login_name_selected = $cgi->param("login_name");
        ...
        $component->set_Template_Default("template_app_admin_multi_select_groups.html");
        
        $dbu->set_Table("$user_group_table_name");
        
        if ($dbu->count_Item("login_name", "$login_name_selected") == 0) {
            $component->set_SQL("select * from $group_table_name order by group_name");
            
        } else {
            my @ahr = $dbu->get_Items("group_name", "login_name", "$login_name_selected");
            my $not_in = undef;
            
            foreach my $item (@ahr) {
                $not_in .= "'$item->{group_name}', ";
            }
            
            $not_in .= "''";
            
            $component->set_SQL("select * from $group_table_name where group_name not in ($not_in) order by group_name");
        }
...
sub customize_TLD {
    ...
    $tld->add_Column("group_name_get_fmt");
    
    my $row_color = "#FFFFFF";
    
    my $group_name_get_fmt = undef;
    ...
    for ($i < 0; $i < $tld->get_Row_Num; $i++) { 
        ...
        $group_name_get_fmt = $tld->get_Data($i, "group_name");
        $group_name_get_fmt =~ s/ /+/g;
        
        $tld->set_Data($i, "group_name_get_fmt", $group_name_get_fmt);
    }
