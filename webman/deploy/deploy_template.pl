...
my $template_dir = $webman_dir . "public_html/cgi-bin/webman/" . $dfi->get_App_Name . "/";

print "\$cur_dir    = $cur_dir\n";
print "\$webman_dir = $webman_dir\n";
print "\$template_dir = $template_dir\n";

print "\nCheck current local webman template files...\n\n"; 

opendir(DOCDIR, $template_dir) || die("Cannot open directory");
...
foreach my $file (@files) {
    if ($file =~ /\.html$/) {
        ...
        my $file_size = -s "$template_dir" . "$file";
        my $file_date_time = localtime(stat("$template_dir" . "$file")->[9]);
...
print "\nCheck current template files on server...\n\n";
...
if (open(MYFILE, "<./logs/template_" . $dns . "-" . $app_name .  "_last.txt")) {
    print "\n######################################################################\n";
    
    print "\nCheck last local webman template files log...\n\n";
...
if ($proceed_task eq "y") {
    ...
    my $mod_info_file_name_last = "template_" . $dns . "-" . $app_name .  "_last.txt";
    my $mod_info_file_name_log  = "template_" . $dns . "-" . $app_name .  "_$date_finish" . "_$time_finish.txt";
    ...
    if (open(MYFILE_LAST, ">./logs/$mod_info_file_name_last") && open(MYFILE_LOG, ">./logs/$mod_info_file_name_log")) {
        ...
        foreach my $key (sort(keys(%file_info_all))) {
            ...
            if ($file_info_all{$key}->{task} eq "Reload" || $file_info_all{$key}->{task} eq "Upload") {
                $ftp_conn->put("$template_dir/$key", $key);
