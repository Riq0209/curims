...
my $mod_app_dir = $webman_dir . "webman/pm/apps/" . $dfi->get_App_Name . "/";

print "\$cur_dir    = $cur_dir\n";
print "\$webman_dir = $webman_dir\n";
print "\$mod_app_dir = $mod_app_dir\n";

print "\nCheck current local webman application modules...\n\n"; 

opendir(DOCDIR, $mod_app_dir) || die("Cannot open directory");
...
foreach my $file (@files) {
    if ($file =~ /\.pm$/) {
        ...
        my $file_size = -s "$mod_app_dir" . "$file";
        my $file_date_time = localtime(stat("$mod_app_dir" . "$file")->[9]);

        #print "$mod_app_dir" . "$file" . "\n";
...
print "\nCheck current application modules on server...\n\n";
...
$ftp_conn->cwd("webman/pm/apps/$app_name") or die "Cannot change working directory ", $ftp_conn->message;
...
foreach my $item (parse_dir($server_files)) {
    ...
    if ($fname =~ /\.pm$/) {
        ...
        print &fmt_num($count) . ". $fname = $map_file_exist_server{$fname}\n";
...
if (open(MYFILE, "<./logs/mod_app_" . $dns . "-" . $app_name . "_last.txt")) {
    print "\n######################################################################\n";
    
    print "\nCheck last local webman application modules log...\n\n";
...
if ($proceed_task eq "y") {
    ...
    my $mod_info_file_name_last = "mod_app_" . $dns . "-" . $app_name . "_last.txt";
    my $mod_info_file_name_log  = "mod_app_" . $dns . "-" . $app_name . "_$date_finish" . "_$time_finish.txt";
    ...
    if (open(MYFILE_LAST, ">./logs/$mod_info_file_name_last") && open(MYFILE_LOG, ">./logs/$mod_info_file_name_log")) {
        ...
        foreach my $key (sort(keys(%file_info_all))) {
            ...
            if ($file_info_all{$key}->{task} eq "Reload" || $file_info_all{$key}->{task} eq "Upload") {
                $ftp_conn->put("$mod_app_dir/$key", $key);
