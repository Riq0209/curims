...
sub customize_TLD_DB_Operation {
    ...
    if (defined($tld)) {
        ...
        __add_col_linked_table_PK__
        
        for (my $i = 0; $i < $tld->get_Row_Num; $i++) {
            __add_FK_start__
            my $__linked_table_UK__ = $tld->get_Data($i, "__linked_table_UK__");
            $dbu->set_Table("__linked_table_name__");
            my $__linked_table_PK__ = $dbu->get_Item("__linked_table_PK__", "__linked_table_UK__", "$__linked_table_UK__");
            $tld->set_Data($i, "__linked_table_PK__", $__linked_table_PK__);
            __add_FK_end__
        }        
    }
