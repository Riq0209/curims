...
sub run_Task {
    ...
    foreach my $param_name (@cgi_var_list) {
        if ($param_name =~ /^__delete_key_field___/) {
            my $__delete_key_field__ = $cgi->param_Shift($param_name);

            #$cgi->add_Debug_Text("$this->{table_name} : $param_name = $__delete_key_field__", __FILE__, __LINE__);
            
            $dbu->set_Table($this->{table_name});
            $dbu->delete_Item("__delete_key_field__", $__delete_key_field__);
        }
    }
