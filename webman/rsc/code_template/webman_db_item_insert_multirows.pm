...
sub run_Task {
    ...
    if ($cgi->param("__irn_cgi_var_name__") > 0) {
        $this->{row_num} = $cgi->param("__irn_cgi_var_name__");
        
    } else {
        $cgi->push_Param("__irn_cgi_var_name__", $this->{row_num});
    }    
...
sub customize_CGI_Data { ### 11/10/2011
    ...
    for (my $row_idx = 0; $row_idx < $this->{row_num}; $row_idx++) {
        ...
        $dbu->set_Table("__linked_table_name__");
        my $__linked_table_PK__ = $dbu->get_Item("__linked_table_PK__", "__linked_table_UK__", $cgi->param("\$db___linked_table_UK___" . $row_idx));            
        $cgi->push_Param("\$db___linked_table_PK___" . $row_idx, $__linked_table_PK__);        
