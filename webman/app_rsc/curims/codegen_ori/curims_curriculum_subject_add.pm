package curims_curriculum_subject_add;
...
sub run_Task {
    ...
    if ($submission_type eq "parent_list") {
        $cgi->push_Param("task", "curims_curriculum_subject_list");        
        $cgi->redirect_Page("index.cgi?$get_data");
        
    } elsif ($submission_type eq "add_selected" && grep(/^id_subject_62base_/, @cgi_var_list)) {  
        my $id_item_parent = $cgi->param("id_curriculum_62base");
        
        $cgi->push_Param("\$db_id_curriculum_62base", $id_item_parent);
        ...
        foreach my $var (@cgi_var_list) {                
            if ($var =~ /^id_subject_62base_/) {
                my $id_item_added = $cgi->param_Shift($var);
                
                $cgi->push_Param("\$db_id_subject_62base", $id_item_added);

                ### get the current number of selected item to be added
                my $num = $var;
                   $num =~ s/^id_subject_62base_//;
                ...
                $cgi->param_Shift("\$db_id_subject_62base");
            }                
        }
