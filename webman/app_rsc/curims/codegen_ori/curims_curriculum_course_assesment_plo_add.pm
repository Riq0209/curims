package curims_curriculum_course_assesment_plo_add;
...
sub run_Task {
    ...
    if ($submission_type eq "parent_list") {
        $cgi->push_Param("task", "curims_curriculum_course_assesment_plo_list");        
        $cgi->redirect_Page("index.cgi?$get_data");
        
    } elsif ($submission_type eq "add_selected" && grep(/^id_plo_62base_/, @cgi_var_list)) {  
        my $id_item_parent = $cgi->param("id_assesment_62base");
        
        $cgi->push_Param("\$db_id_assesment_62base", $id_item_parent);
        ...
        $htmldb->set_Table("curims_assesplo");        
        ...
        foreach my $var (@cgi_var_list) {                
            if ($var =~ /^id_plo_62base_/) {
                ...
                my $percentage = $cgi->param_Shift("percentage_" . $num);
                $cgi->push_Param("\$db_percentage", $percentage);                
...
sub process_DYNAMIC { ### TE_TYPE_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    if ($te_type_name eq "parent_item_info") {
        ...
        #$component->set_SQL("select * from __parent_table_name__ where id_assesment_62base='$id_item_parent'");
