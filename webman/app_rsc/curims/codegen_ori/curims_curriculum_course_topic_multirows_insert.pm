package curims_curriculum_course_topic_multirows_insert;
...
sub run_Task {
    ...
    if ($cgi->param("irn_curims_topic") > 0) {
        $this->{row_num} = $cgi->param("irn_curims_topic");
        
    } else {
        $cgi->push_Param("irn_curims_topic", $this->{row_num});
    }    
