package curims_plo_currplo_link;
...
sub customize_SQL {
    ...
    if (!$cgi->param_Exist("filter_plo_code") || $cgi->param("filter_plo_code") eq "") {
        $cgi->push_Param("filter_plo_code", "\%");
    }
    my $filter_plo_code = $cgi->param("filter_plo_code");
    $sql_filter .= "plo_code like '$filter_plo_code' and ";
...
sub customize_TLD {
    ...
    for ($i = 0; $i < $tld->get_Row_Num; $i++) { 
        ...
        my $count_item = $dbu->count_Item("id_plo_62base", $tld->get_Data($i, "id_plo_62base"));
        $tld->set_Data($i, "curims_currplo", $count_item);
