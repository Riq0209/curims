package curims_currcourse_course_add;
...
sub run_Task {
    ...
    my $id_course_62base = $dbu->get_Item("id_course_62base", "id_currcourse_62base", $cgi->param("id_currcourse_62base"));
    $cgi->push_Param("id_course_62base", "$id_course_62base");
    ...
    if ($submission_type eq "parent_list") {
        $cgi->push_Param("task", "curims_currcourse_course_list");        
...
sub process_DYNAMIC { ### TE_TYPE_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    if ($te_type_name eq "parent_item_info") {
        ...
        #$component->set_SQL("select * from __parent_table_name__ where id_currcourse_62base='$id_item_parent'");
