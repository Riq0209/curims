package curims_curriculum_course_multirows_insert;
...
sub run_Task {
    ...
    if ($cgi->param("irn_curims_course") > 0) {
        $this->{row_num} = $cgi->param("irn_curims_course");
        
    } else {
        $cgi->push_Param("irn_curims_course", $this->{row_num});
    }    
