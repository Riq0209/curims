package curims_lecturer_course_add;
...
sub run_Task {
    ...
    if ($submission_type eq "parent_list") {
        $cgi->push_Param("task", "curims_lecturer_course_list");        
        $cgi->redirect_Page("index.cgi?$get_data");
        
    } elsif ($submission_type eq "add_selected" && grep(/^id_course_62base_/, @cgi_var_list)) {  
        my $id_item_parent = $cgi->param("id_lecturer_62base");
        
        $cgi->push_Param("\$db_id_lecturer_62base", $id_item_parent);
        ...
        $htmldb->set_Table("curims_course_lecturer");        
...
sub process_DYNAMIC { ### TE_TYPE_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    if ($te_type_name eq "parent_item_info") {
        ...
        #$component->set_SQL("select * from __parent_table_name__ where id_lecturer_62base='$id_item_parent'");
...
sub customize_SQL {
    ...
    if (!$cgi->param_Exist("filter_course_code_afls") || $cgi->param("filter_course_code_afls") eq "") {
        $cgi->push_Param("filter_course_code_afls", "\%");
    }
    my $filter_course_code_afls = $cgi->param("filter_course_code_afls");
    $sql_filter .= "course_code like '$filter_course_code_afls' and ";
    
    
    if (!$cgi->param_Exist("filter_course_name_afls") || $cgi->param("filter_course_name_afls") eq "") {
        $cgi->push_Param("filter_course_name_afls", "\%");
    }
    my $filter_course_name_afls = $cgi->param("filter_course_name_afls");
    $sql_filter .= "course_name like '$filter_course_name_afls' and ";
