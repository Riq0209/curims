package curims_curriculum_plo_multirows_insert;
...
sub run_Task {
    ...
    if ($cgi->param("irn_curims_plo") > 0) {
        $this->{row_num} = $cgi->param("irn_curims_plo");
        
    } else {
        $cgi->push_Param("irn_curims_plo", $this->{row_num});
    }    
