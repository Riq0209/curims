package curims_subject_curriculum_add;
...
sub run_Task {
    ...
    if ($submission_type eq "parent_list") {
        $cgi->push_Param("task", "curims_subject_curriculum_list");        
        $cgi->redirect_Page("index.cgi?$get_data");
        
    } elsif ($submission_type eq "add_selected" && grep(/^id_curriculum_62base_/, @cgi_var_list)) {  
        my $id_item_parent = $cgi->param("id_subject_62base");
        
        $cgi->push_Param("\$db_id_subject_62base", $id_item_parent);
        ...
        $htmldb->set_Table("curims_currsubject");        

        foreach my $var (@cgi_var_list) {                
            if ($var =~ /^id_curriculum_62base_/) {
                my $id_item_added = $cgi->param_Shift($var);
                
                $cgi->push_Param("\$db_id_curriculum_62base", $id_item_added);

                ### get the current number of selected item to be added
                my $num = $var;
                   $num =~ s/^id_curriculum_62base_//;
                ...
                $cgi->param_Shift("\$db_id_curriculum_62base");
            }                
        }
...
sub process_DYNAMIC { ### TE_TYPE_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    if ($te_type_name eq "parent_item_info") {
        ...
        #$component->set_SQL("select * from __parent_table_name__ where id_subject_62base='$id_item_parent'");
