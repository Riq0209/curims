package curims_course_elective_multirows_insert;
...
sub run_Task {
    ...
    if ($cgi->param("irn_curims_elective") > 0) {
        $this->{row_num} = $cgi->param("irn_curims_elective");
        
    } else {
        $cgi->push_Param("irn_curims_elective", $this->{row_num});
    }    
