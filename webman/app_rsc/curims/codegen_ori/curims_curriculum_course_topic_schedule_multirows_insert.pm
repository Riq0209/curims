package curims_curriculum_course_topic_schedule_multirows_insert;
...
sub run_Task {
    ...
    if ($cgi->param("\$db_id_topic_62base") ne $cgi->param("id_topic_62base")) {
        $cgi->push_Param("\$db_id_topic_62base", $cgi->param("id_topic_62base"));
    }
    ...
    if ($cgi->param("irn_curims_schedule") > 0) {
        $this->{row_num} = $cgi->param("irn_curims_schedule");
        
    } else {
        $cgi->push_Param("irn_curims_schedule", $this->{row_num});
    }    
