...
print "+----------------------------------------------+\n";
print "|Empty Webman Application Domain Data DB Tables|\n";
print "+----------------------------------------------+\n\n";
...
print "\n" .
      "This operation will truncate all problem domain data for \"$app_name\" \n" . 
      "application. As a precaution against losing valuable application \n" .
      "data, previous table items of problem domain data will be backuped \n" . 
      "into: $dirpath\n\n";
...
if ($dbt_prefix_name ne "") {
    foreach $item (@tbl) {
        if ($item =~ /^$dbt_prefix_name/) {
            ...
            print "  Backup table: $item -> $file_name\n";
            `mysqldump -c -h $db_host -u $db_user_name -p$db_password $db_name $item > $file_name`;
            
            print "Truncate table: $item\n\n";
            `mysql -h $db_host -u $db_user_name -p$db_password -e "truncate table $item" $db_name`;        
        }
    }
}
