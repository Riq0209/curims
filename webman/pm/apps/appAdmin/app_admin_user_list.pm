...
sub customize_TLD {
    ...
    for ($i < 0; $i < $tld->get_Row_Num; $i++) { 
        ...
        $dbu->set_Table("webman_" . $cgi->param("app_name_in_control") . "_user_group");
        $tld->set_Data($i, "group_num", $dbu->count_Item("login_name", $tld->get_Data($i, "login_name")));
        
        #$cgi->add_Debug_Text("SQL1 = " . $dbu->get_SQL);
        
        $dbu->set_Table("webman_" . $cgi->param("app_name_in_control") . "_comp_auth");
        $tld->set_Data($i, "comp_num", $dbu->count_Item("login_name", $tld->get_Data($i, "login_name")));
