...
sub process_DYNAMIC { ### so_type_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    foreach $item (@ahr) {
        ...
        #$cgi->add_Debug_Text("$date_str | $ym_prev : $ym_str", __FILE__, __LINE__);
        
        if ($ym_prev ne $ym_str && $ym_str ne "0000-00") {
            ### Get monthly total user distinctive by their login name.
	    $dbu->set_Table($session_table);
	    $dbu->set_Keys_Str("login_date like '$ym_str-%'");
	    
	    $total_user = $dbu->count_Item(undef, undef, "login_name");
