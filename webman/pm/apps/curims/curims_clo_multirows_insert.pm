package curims_clo_multirows_insert;
...
sub run_Task {
    ...
    if ($cgi->param("\$db_id_course_62base") ne $cgi->param("id_course_62base")) {
        $cgi->push_Param("\$db_id_course_62base", $cgi->param("id_course_62base"));
    }
    ...
    if ($cgi->param("irn_curims_clo") > 0) {
        $this->{row_num} = $cgi->param("irn_curims_clo");
        
    } else {
        $cgi->push_Param("irn_curims_clo", $this->{row_num});
    }    
...
sub customize_CGI_Data { ### 11/10/2011
    ...
    for (my $row_idx = 0; $row_idx < $this->{row_num}; $row_idx++) {
        # Join T&L Methods checkbox values into comma-separated string
        my $tl_methods_param = "\$db_clo_tl_methods_" . $row_idx;
        my @tl_methods = $cgi->param($tl_methods_param);
        if (@tl_methods > 1) {
            my $joined = join(',', grep { defined && length } @tl_methods);
            $cgi->param($tl_methods_param, $joined);
        }
        ...
        ### by referring to one of current table field that act as a unique keys 
