package curiums_curriculum_details_multirows_insert;
...
sub run_Task {
    ...
    if ($cgi->param("irn_curims_curriculum") > 0) {
        $this->{row_num} = $cgi->param("irn_curims_curriculum");
        
    } else {
        $cgi->push_Param("irn_curims_curriculum", $this->{row_num});
    }    
