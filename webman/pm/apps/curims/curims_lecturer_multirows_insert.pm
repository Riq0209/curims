package curims_lecturer_multirows_insert;
...
sub run_Task {
    ...
    if ($cgi->param("irn_curims_lecturer") > 0) {
        $this->{row_num} = $cgi->param("irn_curims_lecturer");
        
    } else {
        $cgi->push_Param("irn_curims_lecturer", $this->{row_num});
    }    
