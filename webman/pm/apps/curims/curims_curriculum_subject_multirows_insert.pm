package curims_curriculum_subject_multirows_insert;
...
sub run_Task {
    ...
    if ($cgi->param("\$db_id_entry_62base") ne $cgi->param("id_entry_62base")) {
        $cgi->push_Param("\$db_id_entry_62base", $cgi->param("id_entry_62base"));
    }
    ...
    if ($cgi->param("irn_curims_subject") > 0) {
        $this->{row_num} = $cgi->param("irn_curims_subject");
        
    } else {
        $cgi->push_Param("irn_curims_subject", $this->{row_num});
    }    
