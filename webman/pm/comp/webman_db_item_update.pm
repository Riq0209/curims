...
sub new {
    ...
    $this->{last_phase_url_redirect} = undef; ### 12/08/2011
...
sub set_Check_On_Fields_Existence { 
    my $this = shift @_;
...
sub set_Last_Phase_URL_Redirect { ### 12/08/2011
    my $this = shift @_;
...
sub get_Last_Phase_URL_Redirect { ### 12/08/2011
    my $this = shift @_;
    
    return $this->{last_phase_url_redirect};
}
...
sub run_Task {
    ...
    ### try to auto set all possible data of CGI database cache variables
    ...
    if ($this->{last_phase}) { ### a normal last phase ### 17/01/2011
        ...
        if ($this->{last_phase_url_redirect} ne "") {
            ...
            ### redirect will cause the end_Task function in the contact of 
            ### current module not called inside the main controller so need
            ### to call it here
            #$this->end_Task;
            ### 12/03/2012
            ### disable the above since it seems not the correct one
...
sub process_Content {
    ...
    #$cgi->add_Debug_Text("\$this->check_Button_On_Submit(\"proceed_\") = " . $this->check_Button_On_Submit("proceed_"), __FILE__, __LINE__);
...
sub process_DATAHTML { ### so_type_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    if ($te_type_name eq "form_db_field") {
        if (!$this->check_Button_On_Submit("proceed_") &&
            ...
            !$this->check_Button_On_Submit("cancel_")) { ### first time call so need to inject DB fields into CGI data
        ...
        ### 04/06/2014
        ### Next is to help clear the default value for the case the  CGI's
        ### database field data sill not exist.
        
        ### for input type text
        $te_content =~ s/value *= *"\$db_.+_"/value=""/g;

        ### for text area
        $te_content =~ s/> *\$db_.+_ *</></g;        
    }
...
sub inject_DBF_To_CGI { ### so_type_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    $this->{sql_view} = $dbihtml->get_SQL;
    
    if ($this->{sql_debug}) {
        $cgi->add_Debug_Text("SQL View = " .$this->{sql_view}, __FILE__, __LINE__, "DATABASE");    
    }
            ...
            if ($cgi->param("\$db_" . $col_name) eq "") { ### allow developer to overwrite instead of 
                ...
                if ($this->{limit_on_fields_dict} ne "") {
                    if ($this->{limit_on_fields_dict}->{$col_name}) {
                        if (!$cgi->set_Param_Val("\$db_" . $col_name, $col_data)) {
                            $cgi->add_Param("\$db_" . $col_name, $col_data);
                        }
                    }
    ...
    if ($this->{check_on_fields_duplication} ne "") {
        ...
        for ($i = 0; $i < @check_on_fields_duplication; $i++) {
            ...
            if ($this->{fe_hash}->{$fe_name} ne "") { ### other error might has been detected 
                ...
                $keystr .= " and " . $this->contra_Update_Key_Str;
