package webman_db_item_insert_multirows;
...
sub new {
    ...
    $this->{row_num} = undef; ### 30/06/2011
    $this->{row_num_limit} = undef; ### 13/03/2013
    $this->{row_num_param} = undef; ### 16/03/2012
...
sub set_Row_Num {
    ...
    $this->{row_num} = shift @_;
}

### 13/03/2013
sub set_Row_Num_Limit {
    my $this = shift @_;
    
    $this->{row_num_limit} = shift @_;
}

sub set_Row_Num_Param {
    my $this = shift @_;
    
    $this->{row_num_param} = shift @_;
}
...
sub run_Task {
    ...
    ### 13/03/2013
    #$cgi->add_Debug_Text("$this->{row_num} - $this->{row_num_limit}", __FILE__, __LINE__);
    if ($this->{row_num_limit} > 0 && 
        $this->{row_num_limit} < $this->{row_num}) { 
        $this->{row_num} = $this->{row_num_limit}; 
        
        #$cgi->add_Debug_Text("Exceed row number limit", __FILE__, __LINE__);
    }    
    
    ### 16/03/2012
    if ($this->{row_num_param} eq "") {
        $this->{row_num_param} = "irn_" . $this->{table_name};
    }
    
    if ($cgi->param($this->{row_num_param}) > 0 && 
        $cgi->param($this->{row_num_param}) <= $this->{row_num_limit}) {
        $this->{row_num} = $cgi->param($this->{row_num_param});
        
    } else {
        $cgi->push_Param($this->{row_num_param}, $this->{row_num});
    }   
    ...
    if ($this->{table_name} ne "" && $this->{last_phase} && !$this->check_Button_On_Submit("cancel_")) {
        ...
        #$cgi->add_Debug_Text("Try to save item into DB", __FILE__, __LINE__, "TRACING");
        
        ### 17/02/2011
        #if ($this->{row_num} > 1) { ### insert multi row item
            my @cgi_var_list = $cgi->var_Name;
    ...
    #$cgi->add_Debug_Text("check_On_CGI_Data - " . $this->check_On_CGI_Data, __FILE__, __LINE__);
    #$cgi->add_Debug_Text("check_On_Fields_Duplication - " . $this->check_On_Fields_Duplication, __FILE__, __LINE__);
    #$cgi->add_Debug_Text("check_On_Fields_Existence - " . $this->check_On_Fields_Existence, __FILE__, __LINE__);    
...
sub process_DATAHTML { ### so_type_ can be: VIEW, DYNAMIC, LIST, MENU, DBHTML, SELECT, DATAHTML
    ...
    if ($te_type_name eq "form_db_field") {
        ...
        for (my $i = 0; $i < $this->{row_num}; $i++) {
            ...
            #if ($this->{row_num} > 1) { ### handle multi row insert
                ### for input type text
                $content_temp =~ s/value *= *"\$db_.+_$i."/value=""/g;

                ### for text area
                $content_temp =~ s/> *\$db_.+_$i. *</></g;
...
sub check_On_Fields_Duplication {
    ...
    if ($this->{check_on_fields_duplication} ne "") {    
        ...
        for (my $row_idx = 0; $row_idx < $this->{row_num}; $row_idx++) {
            my $row_idx_str = undef;
            
            #if ($this->{row_num} > 1) { ### multi row item insert support
                $row_idx_str = "_$row_idx";
            #}
...
sub generate_Field_Error_Variables_Info {
    ...
    foreach $hash_key (keys(%fe_hash)) {
        ...
        ### field entry (the exact last phase of module process).
